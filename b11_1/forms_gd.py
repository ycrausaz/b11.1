...
#    geschaeftspartner = forms.CharField(required=False)
...
#        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set up foreign key widgets with their querysets
        foreign_key_fields = {
            'basismengeneinheit': Basismengeneinheit,
            'begru': BEGRU,
            'materialart_grunddaten': Materialart,
            'sparte': Sparte,
            'rueckfuehrungscode': Rueckfuehrungscode,
            'serialnummerprofil': Serialnummerprofil,
            'spare_part_class_code': SparePartClassCode,
            'uebersetzungsstatus': Uebersetzungsstatus,
            'gefahrgutkennzeichen': Gefahrgutkennzeichen,
            'werkzuordnung_1': Werkzuordnung_1,
            'werkzuordnung_2': Werkzuordnung_2,
            'werkzuordnung_3': Werkzuordnung_3,
            'werkzuordnung_4': Werkzuordnung_4,
            'allgemeine_positionstypengruppe': AllgemeinePositionstypengruppe,
            'fertigungssteuerer': Fertigungssteuerer,
            'sonderablauf': Sonderablauf,
            'temperaturbedingung': Temperaturbedingung,
            'bewertungsklasse': Bewertungsklasse,
            'materialeinstufung_nach_zuva': MaterialeinstufungNachZUVA,
            'zuteilung': Zuteilung,
            'auspraegung': Auspraegung,
        }

        # Initialize foreign key widgets with proper querysets
        for field_name, model in foreign_key_fields.items():
            if field_name in self.fields:
                if isinstance(self.fields[field_name].widget, ReadOnlyForeignKeyWidget):
                    self.fields[field_name].widget.choices = model.objects.all()
